...
Short demo video: <YOUTUBE_LINK>
Postman file: postman_collection.json (in repo)

# Eterna Order Execution Engine

Eterna is a tiny order execution sandbox: drop in an order, stream every lifecycle update over WebSocket, then read the canonical record from Postgres once the worker wraps up.
